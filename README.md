...
ColorArchive is a static-export-first color product built with Next.js, TypeScript, Tailwind CSS, and the App Router, with a separate Express API for email capture, auth, checkout webhooks, and analytics. It includes the core archive, dense `All Colors`, curated `Collections`, `Families`, local and account-backed browsing tools, public notes and guides, sellable palette packs, and supporting product surfaces such as `Free Pack`, `Thanks`, `Cancel`, and `Product Examples`.
...
- TypeScript
- Tailwind CSS
- Express API + SQLite for dynamic flows
...
- `src/lib/checkout-config.ts` centralizes hosted checkout URLs, provider state, and success/cancel return paths.
- `src/lib/checkout-config.ts` also centralizes waitlist and free-pack capture settings.
...
- The frontend is fully static-export compatible and deployable on GitHub Pages.
- Dynamic flows live in the separate Express API under `server/`.
