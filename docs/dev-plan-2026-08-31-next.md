...
## §9.10 A 的复核:两个模型各挑出一个我确实错了的地方,但结论没变

owner 问「你确定吗」,让 Gemini 3.7 Flash 和 Codex 各复核一遍。**两边都找到了真错误 —— 而且不是同一个。**

### 🔴 Gemini 抓到的:我**整段只算了核数,完全没算内存**

从 Turbo 换 Standard 不只是 30 核 → 4 核,**还是 60 GB → 8 GB**。
而我用来推理的那次 22 秒构建,日志里明写着 `Restored build cache from previous deployment` ——
**是热缓存**。Gemini 的推论:冷构建(Next.js 升级、根 layout 改动、缓存过期)可能又慢又吃内存,
4,461 页的并行 worker 在 8 GB 上有 OOM(exit 137)风险,建议退一步选 Enhanced(8 核 / 16 GB)。

**这一条我确实完全没想到。已实测,结论是不成立,但担心是合理的:**

```
# 本地冷构建(先 rm -rf .next),/usr/bin/time -l
✓ Compiled successfully in 8.8s
✓ Generating static pages using 9 workers (4456/4456) in 23.9s
  46.79 real   116.05 user   32.96 sys
# 单进程峰值 1.25 GB;但 time -l 报的是最大的那个进程,不是进程树之和
# 于是又采样了整棵树:
PEAK AGGREGATE RSS of all node processes: 2.01 GB across 22 processes
```

**峰值 2.01 GB vs Standard 的 8 GB = 4 倍余量。** 而且冷构建里 4,456 页只用了 23.9 秒 ——
**「热缓存让页面生成变成增量」这个前提本身也不成立**,冷的也就这么快。

**更强的一条旁证:GitHub Actions 的 `ubuntu-latest` 就是 4 vCPU,和 Vercel Standard 同核数,
而且没有 Vercel 构建缓存,每次推送都跑完整 `npm run build`:**

| CI run | Production build |
|---|---:|
| 33381899753 | 80s |
| 33378594041 | 57s |
| 33374033714 | 84s |

**三次全过,从没 OOM。** 这基本上就是提前替我们跑过的 Standard 实验。

→ Enhanced 不需要。Codex 的盈亏算法:Enhanced 单价是 Standard 的 2 倍($0.028 vs $0.014/分钟),
**必须把总时长砍掉一半以上才回本**,而 143 秒的 I/O 开销根本不随核数变 —— 不可能。

### 🔴 Codex 抓到的:我自己的两处算术

1. **并行度算错了,而且指标本身用错了。** 我写 `116.05/46.79 = 2.48x`,漏了 sys:
   `(116.05+32.96)/46.79 = **3.18**`。更重要的是,**CPU 时间 ÷ 墙钟 = 平均占用核数,不是加速比** ——
   它不能证明「30 核浪费了 92%」。改正后结论方向不变(冷构建全程平均只占 3.18 个核,
   4 核仍然合适),但我原来的说法是错的。
2. **省的钱我说得含糊。** $0.235/次 × 次数,而次数我在两处用了不同的值。
   ⚠️ Codex 据此算成 **$2.7/月** —— 那是他把「11–12 次构建」当成了每月,
   **而那其实是账期至今 6 天的数**(≈58 次/月)。**这是我简报写得不清楚,不是他的错。**
   实际区间:
   ...
   | 12 次/月(很安静) | $2.8 |
   | 40 次/月 | $9.4 |
   | 58 次/月(当前 6 天的速率) | $13.6 |
   | 90 次/月(8 月的实际速率) | $21.2 |

   **所以「约 $11/月」是个合理的中位数,但它随活跃度浮动,不是一个定数。**

### Codex 解释了那 60 秒空档,而且核对无误

不是没记录的 CPU 工作,是 **Vercel 的 Build Output API 在收尾**:枚举、哈希、写入产物清单。
他估计 `.next` 有约 46,800 个文件 —— 实测:

```
.next 共 46,832 个文件 · .next/server/app 45,522 个 · 1.2 GB
```

单线程遍历+哈希 46,832 个文件花 60 秒,完全说得通。**这段几乎不随核数变。**

### 结论:即使按最坏情况算,决定也不翻

| 情形 | 预计总时长 | CPU-分钟 | 每次成本 |
|---|---:|---:|---:|
| CI 最好(57s 构建段) | 3.3 min | 13.3 | $0.047 |
| CI 最差(84s 构建段) | 3.8 min | 15.1 | $0.053 |
| **Codex 的最坏假设**(那 60 秒**也**按 7.5 倍膨胀) | **10.3 min** | 41.1 | $0.144 |
| Turbo 实测 | 2.75 min | 82.5 | **$0.289** |
| **4 核盈亏平衡线** | **20.6 min** | — | — |

**连 Codex 自己构造的最坏情况都只有 10.3 分钟 —— 仍是平衡线的一半,仍比 Turbo 便宜 50%。**
所以那个还没拿到的实测数只能修正预测,**不可能推翻决定**。

### 还没做的那次测量,以及为什么不再追

Codex 指出 Vercel 的 Redeploy 弹窗可以**取消勾选 "Use project's Ignore Build Step"**,
这样就能绕过 `ignoreCommand` 跑一次冷的 Standard 构建 —— **这个我原先不知道,是个好办法。**
但 Vercel 面板在这台机器上反复卡在骨架屏(即使负载已降到 5),试了多次拿不到。

**不追了,因为下一次真实代码推送会免费给出这个数**(autopilot 往 `src/lib/` 加内容也算)。
判据仍然是:**< 15 分钟留着,> 20.6 分钟换回 Elastic**。
两个模型独立给的期望值分别是 4–5 分钟和 4–7 分钟,而我的预测是 3.3–3.8 分钟。

### 各自错在哪(留档,因为这决定了下次该怎么用它们)

- **Gemini**:抓到了我最大的盲点(内存),但它的两个论据 ——「热缓存掩盖了冷构建代价」和
