...
## §9.12 「上个月换了能省多少」—— 我和 Gemini 3.7 各算一遍,结果一致

owner 问 8 月如果用 Standard 能省多少。**两边独立算,数字几乎重合:**

| | 我 | Gemini 3.7 Flash |
|---|---:|---:|
| 8 月构建次数 | 90.2 | 90.2 |
| Standard 月费(中位) | $4.69 | $4.72 |
| **省** | **$21.48(82.1%)** | **$21.45(82.0%)** |
| 最好(构建段 57s) | 省 $21.96 / 83.9% | 省 $21.94 / 83.8% |
| 极端最坏(那 60s 也按 7.5× 膨胀) | 省 $13.19 / 50.4% | 省 $13.12 / 50.1% |

推导:Turbo 单次 `30 核 × 165s = 4,950 核·秒 = 1.375 核·小时`;`124h ÷ 1.375 = 90 次`。
Standard 单次 `4 × (143 + 80) = 892 核·秒`,是原来的 **18.0%**。
单价核对 `$26.17/124h = $0.2110/核·小时`,与 Vercel 公布的 $0.0035/CPU-分钟 = $0.210 吻合。

**放到整张账单上:8 月总计 $99.49,省 $21 ≈ 全账单的 21%。**
⚠️ 我上一轮说「约 $11/月、约 11%」是**用了约 50 次/月的构建数**,而 8 月实际 90 次。
**这是本会话里构建次数第三次把估算带偏(前两次见 §9.7、§9.10),应该早点把它钉死。**

代价:全月累计多等 **53–93 分钟**(极端情况 679 分钟)。单人项目、git push 异步部署,基本无感。

### 🔴 但这个数不能直接外推到下个月,原因比 Gemini 列的更要命

Gemini 列的陷阱(次数波动、冷缓存、GHA 与 Vercel 硬件差异)都成立,但它不知道最关键的一条:

**8 月那 90 次构建本身就是虚高的。** `vercel-ignore.sh` 当时 fail-open,
光一个窗口就有 **14 次一次性分支构建**(提交信息全是「no new emails」),每次白建 4,461 页。
该 bug 08-26 已修 —— 最近 20 次部署 **11 次被正确跳过**。

所以「上个月能省 $21」把**两个修复的功劳混在了一起**:换机器(在评估的)和修构建次数(已完成,且是更大的杠杆)。

按每次省 **$0.237** 算,往后的实际节省:

| 构建频率 | 每月省 |
|---|---:|
| 90 次(8 月,含 bug 浪费) | $21.3 |
| 58 次(当前 6 天速率) | $13.7 |
| 40 次 | $9.5 |
| 12 次(很安静) | $2.8 |

**结论:回答「上个月」是 ~$21;作为对下个月的预期,$9–14 更诚实。**
👉 **09-25 核对账期时,Build CPU 这一项应落在 $5–10 区间**(次数下降 × 单价下降双重作用)。
若明显高于此,先查构建次数而不是怀疑机器设置。

**Gemini 的两处小问题**(留档):它又提了一次 8 GB OOM 风险 —— 本轮我只给了它成本数据,
内存那条 §9.10 已实测掉(峰值 2.01 GB / 8 GB);它写「全月多等 58 分钟」应为 **87 分钟**
(把 58 秒/次写成了分钟),后面的 678 分钟是对的。

---

## §9.13 还能省什么 —— 顺带把 8 月两笔大账**提前验了**(不必等 09-25)

owner 问「还有别的能省钱的方式吗」。找的过程里发现:**Vercel 上真正该验的东西现在就能验**,
不用等 09-25 —— Vercel 的 runtime logs 按 `source` 分组是可读的。

### 🔵 好消息:8 月最大的两笔,实测已经塌下去了

近 24 小时生产环境按来源分:

| source | 次/天 | 折算/月 |
|---|---:|---:|
| redirect | 123,098 | ~3.7M |
| rewrite | 105,215 | ~3.2M |
| cache | 86,701 | ~2.6M |
| **function**(真渲染 + ISR 写入) | **1,301** | **~39K** |

对比 8 月:

| 项 | 8 月 | 现在(外推) | 降幅 | 原费用 |
|---|---:|---:|---:|---:|
| ISR 写入 | 8,750,000 | ~39,000 | **99.6%** | $34.99 |
| Edge 请求 | 16,200,000 | ~9,450,000 | 41.7% | $15.09 |
| Build CPU | 124 核·小时 | ~24 核·小时 | 81% | $26.17 |

⚠️ **口径说明**:1,301 是**所有** function 调用(含 `/api/*`、OG 图渲染),
所以它是 ISR 写入的**上限**,真实值只会更低。即便按上限算也是 99.6% 的降幅。

Edge 请求 ~9.45M **落在 10M 免费额度以内** → 那 $15.09 大概率归零。

**账单预测:$99.49(=$20 底价 + $79.49 超额) → 约 $20–25。**
👉 这比我在 §9.12 写的「Build CPU 落在 $5–10」更进一步:**整张账单**都该回到底价附近。
09-25 核对时若不是这样,先查 edge 请求有没有越过 10M,再查构建次数。

### `/vs/` 的余波:已是最廉价形态,不用再动

实测 `https://colorarchive.org/colors/moss-silk-vivid/vs/amber-pearl-muted/` → **HTTP 308,0.13 秒**,
不渲染、不写 ISR。robots.txt 线上确认 `Disallow: /colors/*/vs/` 生效。
爬虫仍在敲(redirect 12.3 万/天里有它的份,另一部分是 `trailingSlash: true` 造成的 308),
但每次只是一个边缘请求。**§9.7 记的「等一个账期再解封让 Google 看 301」这个计划不变。**

### 结论:Vercel 这条线已经挤干了

剩下的都是小钱:`/colors/[slug]` 是全站唯一还开着 `dynamicParams = true` 的路由
(其余 12 个动态路由全是 `false`),但它现在每天只产生千级的 function 调用,**不值得为它冒险改**
(改成全预渲染会撞 Vercel 80MB 部署产物上限,见 `app/colors/[slug]/page.tsx:35`)。

### 🔴 但把视角放大一层:Vercel 修完后只占总支出的 8%

按 `~/Documents/credits.md`:

| 项 | 每月 | 占比 |
|---|---:|---:|
| **Claude Max 20×** | **$200.00** | **79.9%** |
| OpenAI Plus | $22.00 | 8.8% |
| Vercel Pro | $20.00 | 8.0% |
| Apple Developer($99/年) | $8.25 | 3.3% |
| **合计** | **$250.25** | |

云资源目前 **$0**:Azure 走学生额度(¥9,958,到期 **2027-03-18**),DO droplet 08-30 已销毁。
⚠️ **2027-03-18 之后 Azure 那台机器要开始真花钱**(约 ¥600–670/月,唯一收费项是静态公网 IPv4 ¥598/月)。

**所以:两个 AI 订阅占了 88.7%,而我这两天优化的是那 8%。**
这不是说 Vercel 的活白干(它从 $99 降到 $20,是实打实的),
而是**如果目标是「让支出配得上收入」,数量级不在基础设施上** —— 历史总收入是 2 笔外部付费。
那两个订阅是不是该留,是产品/工作方式的决定,不是成本优化能回答的,**留给 owner**。
