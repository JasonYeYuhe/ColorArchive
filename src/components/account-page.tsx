    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-xs text-slate-500 dark:text-slate-400">
          {isUnlimited ? (
            <span className="text-slate-700 dark:text-slate-300 font-semibold">{t("account.unlimited")}</span>
          ) : (
            <>{used} / {limit}</>
          )}
        </span>
        ...
        <div className="h-2 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${
              isNearLimit ? "bg-orange-500" : "bg-neutral-400 dark:bg-neutral-500"
            }`}
            style={{ width: `${pct}%` }}
          />
        </div>
...
function ApiKeySection() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/me/api-key`, { credentials: "include" })
      .then((r) => r.json())
      .then((d) => { setHasKey(d.hasKey); setPrefix(d.prefix); })
      .catch(() => {});
  }, []);
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      if (data.apiKey) {
        ...
        setHasKey(true);
        setPrefix(data.apiKey.slice(0, 7) + "...");
      }
    } catch {
      // ignore
    } finally {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-100 dark:border-white/10 shadow-sm p-6">
      ...
      {newKey ? (
        <div>
          <p className="text-xs text-amber-600 dark:text-amber-400 mb-2">{t("account.apiKeyCopyWarning")}</p>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-mono text-slate-500 dark:text-slate-400">{prefix}</span>
          <button
            onClick={generateKey}
            disabled={loading}
            className="px-3 py-1.5 text-xs font-semibold rounded-xl border border-slate-200 dark:border-white/15 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
          >
            {t("account.regenerateApiKey")}
          </button>
          <button
            onClick={revokeKey}
            className="px-3 py-1.5 text-xs font-semibold rounded-xl text-red-600 dark:text-red-400 border border-red-200 dark:border-red-400/30 hover:bg-red-50 dark:hover:bg-red-400/10 transition-colors"
          >
            {t("account.revokeApiKey")}
          </button>
        </div>
      ) : (
        <button
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-neutral-950 dark:to-neutral-900 flex items-center justify-center p-4">
        <div className="text-center max-w-sm space-y-4">
          <h1 className="text-2xl font-display font-light text-slate-900 dark:text-white">{t("account.signIn")}</h1>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-100 dark:border-white/10 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {!isPro && (
              <Link
                href="/pro/"
                className="px-4 py-2 bg-neutral-900 text-white text-xs font-semibold rounded-xl hover:bg-neutral-700 transition-colors dark:bg-white dark:text-neutral-950 dark:hover:bg-neutral-200"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={href}
                href={href}
                className="flex items-center justify-between p-3 rounded-xl border border-slate-100 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-colors"
              >
