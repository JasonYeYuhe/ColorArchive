...
This file provides guidance to coding agents working with code in this repository. It mirrors the current `CLAUDE.md` so Codex can use the same project instructions without migrating or removing the original file.
...
**Preferred: use the wrapper script** (`~/Library/Scripts/colorarchive-session-lock.sh`). Its command-line surface is stable, so "Always allow" sticks and no permission prompt fires on subsequent runs:
```bash
bash ~/Library/Scripts/colorarchive-session-lock.sh acquire <holder>
# holder values: autopilot | remote | rotation-a | weekly-roundup | etc.
# Exit codes: 0 = acquired; 1 = held by someone else; 2 = IO error
```
The wrapper handles stale-lock (>20 min) detection automatically.

**Fallback (deprecated):** if the wrapper script is missing, write the lock file directly — but expect repeated permission prompts:
```bash
...
**Every session MUST automatically release the lock after its final commit+push.** This is not optional — do it immediately after pushing, before ending the session.

**Preferred: wrapper script:**
```bash
bash ~/Library/Scripts/colorarchive-session-lock.sh release
```
